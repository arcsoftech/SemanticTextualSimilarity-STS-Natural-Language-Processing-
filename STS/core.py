...
def training(devset):
    ...
    devset['cosineScore'] = devset.apply(lambda row: cosine_simlarity(featureObject.sent_vector(row.Sentence1),featureObject.sent_vector(row.Sentence2))
                            , axis = 1)
...
def testing(model,testset):
    ...
    testset['cosineScore'] = testset.apply(lambda row: cosine_simlarity(featureObject.sent_vector(row.Sentence1),featureObject.sent_vector(row.Sentence2))
                            , axis = 1)
